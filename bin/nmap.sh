#!/bin/sh
builtin command -v nmap > /dev/null 2>&1 || { echo 'Error: "nmap" is not installed.' >&2; exit 1; }
builtin command -v xsltproc > /dev/null 2>&1 || { echo 'Error: "xsltproc" is not installed.' >&2; exit 1; }
OUTPUT=$(echo $1 | tr '/' '_')
